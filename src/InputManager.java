...
public class InputManager implements KeyListener, MouseListener, MouseMotionListener, MouseWheelListener {
	 ...
	 * Most of the codes (except for some rare ones like "alt graph") are less than 600.
	 */
	...
	/* Sets whether relative mouse mode is on or not. For relative mouse mode, the mouse is
	 * "locked" in the center of the screen, and only the change in mouse movement is measured.
	...
	/* Maps a GameAction to a specific key. The key codes are defined in java.awt.KeyEvent. If the
	 * key already has a GameAction mapped to it, the new GameAction overwrites it.
	 */
	 ...
	 * (MOUSE_MOVE_LEFT, MOUSE_BUTTON_1, etc). If the mouse action already has a GameActionMapped to it,
	...
	public void clearMap(GameAction gameAction) {
		...
		for(int j = 0; j < mouseActions.length; j++) {
			if(mouseActions[j] == gameAction) {
				mouseActions[j] = null;
			}
		}
	...
	/* Gets a List of names of the keys and mosue actions mapped to this GameAction. Each entry in the List
	 * is a String.
	 */
	public List getMaps(GameAction gameCode) {
		...
		for(int j = 0; j < mouseActions.length; j++) {
			if (mouseActions[j] == gameCode) {
				list.add(getKeyName(j));
			}
		}
	...
	public void resetAllGameActions() {
		...
		for (int j = 0; j < mouseActions.length; j++) {
			if(mouseActions[j] != null) {
				mouseActions[j].reset();
			}
		}
	}
